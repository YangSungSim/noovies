...
export const moviesApi: MovieFetchers = {
    ...
    search: ({ queryKey }) => {
        ...
        return fetch(
            `${BASE_URL}/search/movie?api_key=${API_KEY}&language=en-US&page=1&query=${query}`).then(res => res.json());
    },
    detail: ({ queryKey }) => {
        const [_, id] = queryKey;
        return fetch(
...
export const tvApi: TVFetchers = {
    ...
    search: ({ queryKey }) => {
        const [_, query] = queryKey;
        return fetch(
            `${BASE_URL}/search/tv?api_key=${API_KEY}&language=en-US&page=1&query=${query}`).then(res => res.json());
    },
    detail: ({ queryKey }) => {
        const [_, id] = queryKey;
        return fetch(
